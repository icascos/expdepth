...
extreme.points <- function(data,n=1000,alpha=0.15){
...
BExPlot <- function(data,n=1000,conf=0.95) {
  plot(data,type="n",main="BExPlot",xlab="X",ylab="Y")
  av <- colMeans(data)
  bag <- extreme.points(data,n=n)
  alpha.conf <- conf.level(beta=conf,n=nrow(data))
  conf.region <- extreme.points(data,n=n,alpha=alpha.conf)
